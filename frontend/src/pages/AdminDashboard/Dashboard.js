...
const Dashboard = () => {
  ...
  const toggleModalChart = () => {
    // setIsModalChartOpen(!isModalChartOpen);
    setIsModalChartOpen((prev) => !prev);
  };

  const toggleModalRanking = (ranking) => {
  ...
  const [totalViewsPerMovie, setTotalViewsPerMovie] = useState([]);
  ...
  useEffect(() => {
    if (movieStats.length === 0) return; // Tránh chạy nếu không có dữ liệu
    ...
    setTotalViewsPerMovie(moviesWithTotalViews);
    // console.log('moviesWithTotalViews:', moviesWithTotalViews);
                ...
                <table className="table rounded m-0">
                  <tbody className="text-center align-middle">
                    {currentData.map((movie, index) => (
                      <tr key={movie.movieId}>
                        {/* <td className='index-table-modal-ranking'>1000</td> */}
